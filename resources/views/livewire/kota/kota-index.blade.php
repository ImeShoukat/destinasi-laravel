<div class="max-w-6xl mx-auto px-4 py-6">
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-3xl font-semibold text-zinc-800 dark:text-zinc-100">Manajemen Kota</h1>
        <a href="{{ route('kota.create') }}" wire:navigate
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            <thead class="bg-zinc-100 dark:bg-zinc-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-zinc-600 dark:text-zinc-300">Nama Kota</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse($kotas as $index => $kota)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700">
                        ...
                        <td class="px-6 py-4 text-sm font-medium text-zinc-900 dark:text-white">{{ $kota->nama_kota }}</td>
                        <td class="px-6 py-4 text-center space-x-2">
                            <a href="{{ route('kota.edit', $kota->id) }}" wire:navigate
                               class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white text-sm px-3 py-1 rounded shadow transition">
                                Edit
                            </a>
                        <button 
                                onclick="if(confirm('Apakah Anda yakin ingin menghapus ulasan ini?')) { @this.call('delete', {{ $kota->id }}) }"
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-4 text-center text-zinc-500 dark:text-zinc-400 text-sm">Belum ada data kota.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
